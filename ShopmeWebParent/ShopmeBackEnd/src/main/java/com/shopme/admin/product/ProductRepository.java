...
public interface ProductRepository extends SearchRepository<Product, Integer> {
	...
	public Page<Product> searchInCategory(Integer categoryId, String categoryMatch,
			String keyword, Pageable pageable);
	
	/*
	@Query("SELECT c FROM Category c WHERE c.parent.id is NULL")
	public List<Product> findRootCategories(Sort sort);
	
	@Query("SELECT c FROM Category c WHERE c.parent.id is NULL")
	public Page<Product> findRootCategories(Pageable pageable);
